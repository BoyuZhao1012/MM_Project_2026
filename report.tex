...
\author{PB23071363 赵博宇// PB23010319 刘浩哲 // PB23000052 杨硕}
\date{\today}
...
\hline
PB23071363 & 赵博宇 & 负责项目基础框架与核心建模。搭建捕食者—猎物动力学模型的整体程序结构，实现基础模型、恐惧效应模型与记忆效应模型并基于四阶Runge-Kutta方法完成数值求解；完成平衡点与稳定性（Jacobi特征值）分析、参数扫描、分岔图、相图、零斜率线与敏感性分析等基础实验，并搭建基于tkinter和matplotlib的交互式UI界面与报告初稿。 \\
\hline
PB23010319 & 刘浩哲 & 负责模型创新拓展与版本整合。完成三项创新——基于真实猞猁—雪兔数据的定量参数估计、引入环境噪声的随机微分方程灭绝风险分析、以及恐惧生理代价导致种群崩溃的机制建模（解析临界恐惧强度$f_c=r/\kappa$）；修复基础代码缺陷，补充平衡点验证、振荡周期与二维稳定性分析，并完成各版本代码合并与报告创新章节撰写。 \\
\hline
PB23000052 & 杨硕 & 负责恐惧—记忆二维参数灭绝风险实验与成果汇报。设计并实现恐惧强度$f$与记忆速率$\alpha$的二维参数扫描，绘制猎物低密度灭绝风险图，揭示两因素的耦合效应；整理实验图表，并负责最终的课堂汇报展示。 \\
\hline
\end{tabular}
\end{table}
...
\subsection{实验八：恐惧—记忆二维参数灭绝风险图}

实验三表明记忆速率$\alpha$会显著改变记忆模型的动力学。为进一步刻画恐惧强度与
记忆效应\textbf{共同作用}下的临界风险区域，本节对记忆模型做二维参数扫描。具体地，
令恐惧强度$f \in [0,5]$、记忆速率$\alpha \in [0.05,2.5]$，对每一组参数运行数值模拟，
并在去除前30\%暂态过程后记录猎物密度的最低值$x_{\min}$。当$x_{\min}<0.2$时，
将该参数组合定义为高灭绝风险区域。这里的阈值并不表示确定性灭绝，而是表示猎物
种群进入极低密度状态；在真实生态系统中，此类低密度状态更容易受到随机扰动、疾病
或环境变化影响而发生实际崩溃（与创新二的随机灭绝分析互为补充）。

图\ref{fig:exp11_risk}展示了$f$和$\alpha$共同变化时的风险分布。可以看到，恐惧强度
和记忆速率并非独立发挥作用：当记忆反馈较慢或中等时，系统可能长时间低估或滞后响应
捕食压力，导致猎物低谷显著下降；而较快的记忆更新使$M(t)$更接近当前捕食者密度，
系统行为更接近瞬时恐惧模型，极低密度风险有所缓和。在本组参数范围内，最低猎物密度
约在$0.065$到$0.833$之间变化，按$x_{\min}<0.2$判定时，高风险参数组合约占53.8\%。
这说明恐惧强度与记忆速率之间存在明显耦合，二维参数图比单参数扫描更能揭示潜在的
临界风险区域。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{pics/exp11_extinction_risk_heatmap.pdf}
    \caption{恐惧强度$f$与记忆速率$\alpha$共同作用下的猎物低密度风险图}
    \label{fig:exp11_risk}
\end{figure}
